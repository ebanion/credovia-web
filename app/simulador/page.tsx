...
export default function SimuladorPage() {
  ...
  const calculateMortgage = () => {
    // 1. Calculate Expenses (Impuestos y gastos)
    // Simplified logic: ~10% for used, ~11.5% for new
    const expenseRate = propertyType === "new" ? 0.115 : 0.09
    const expenses = Math.round(propertyPrice * expenseRate)

    // 2. Calculate Loan Amount (Importe a financiar)
    // Formula: Total Needed = Price + Expenses
    // Loan = Total Needed - Savings
    // However, usually banks lend based on Price, not Price+Expenses.
    // If Savings < Expenses, you need more savings? 
    // Let's assume the user pays expenses + down payment from savings.
    // Wait, the reference image shows:
    // Price: 300k, Savings: 80k
    // Expenses: 35.050 (~11.6%)
    // Loan: 255.050
    // Math check: 300k + 35.050 - 80k = 255.050. Correct.
    // So Loan = (Price + Expenses) - Savings.
    
    const loanAmount = (propertyPrice + expenses) - savings

    // 3. Calculate Down Payment (Entrada)
    // The part of the house price paid by savings (after paying expenses)
    // Entrada = Savings - Expenses
    let downPayment = savings - expenses
    if (downPayment < 0) downPayment = 0 // Should alert user ideally

    // 4. Calculate Monthly Quota (Fixed Rate Example)
    ...
    <div className="min-h-screen bg-slate-50 py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-3xl font-bold text-slate-900">Calcula la cuota de tu hipoteca</h1>
        </div>
